...
export const ReportsDashboard: React.FC<ReportsDashboardProps> = ({ classes, schedule }) => {
  ...
  let totalPresent = 0;
  let totalRecords = 0;
  let absentees: {name: string, class: string, date: string}[] = [];
  ...
  classes.forEach(c => {
    c.students.forEach(s => {
        ...
        dates.forEach(date => {
            ...
            if (s.attendance[date] === 'absent') absentees.push({name: s.name, class: c.name, date});
        });
    });
  });
  ...
  const exportToCSV = () => {
    ...
    csvContent += "اسم الفصل,اسم الطالب,المشاركة,ملاحظات\n";
    
    classes.forEach(c => {
        c.students.forEach(s => {
            csvContent += `${c.name},${s.name},${s.participationScore},"${s.notes}"\n`;
        });
    });
    ...
    link.setAttribute("download", `تقرير_الطلاب_${new Date().toLocaleDateString()}.csv`);
  ...
  return (
    <div className="h-full p-4 md:p-6 flex flex-col gap-6 overflow-y-auto pb-24 lg:pb-6 custom-scrollbar">
      <div className="flex items-center justify-between mb-2">
        ...
        <button 
            onClick={exportToCSV}
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              <div className="relative pt-1">
                  <div className="flex mb-2 items-center justify-between">
                      <span className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-primary bg-primary/10">تم التحضير</span>
                      <span className="text-xs font-semibold inline-block text-primary">{planningProgress}%</span>
                  </div>
                  <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-slate-100 dark:bg-slate-700">
                      <div style={{ width: `${planningProgress}%` }} className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-primary transition-all duration-1000"></div>
                  </div>
              </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
              <h3 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                  <AlertTriangle className="w-5 h-5 text-red-500" /> سجل الغياب (آخر 5)
              </h3>
              {absentees.length > 0 ? (
                  <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar">
                      {absentees.slice(-5).reverse().map((record, idx) => (
                  ...
                  <div className="flex flex-col items-center justify-center h-32 text-slate-400">
                      ...
                      <p className="text-sm">لا توجد حالات غياب مسجلة</p>
                  </div>
              )}
          </div>
      </div>
...
const StatCard = ({ title, value, icon, color, subtext }: { title: string, value: string | number, icon: React.ReactNode, color: string, subtext?: string }) => (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-start justify-between relative overflow-hidden group">
        ...
        <div className={`p-3 rounded-xl shadow-lg ${color} transform group-hover:scale-110 transition-transform duration-300 relative z-10`}>
            {icon}
        </div>
    </div>
);

...
import { ClipboardList, Star, Search, Plus, Minus, BookOpen, PenTool, Lightbulb, X, SlidersHorizontal, Award, Zap, Save, Settings2 } from 'lucide-react';
...
interface StudentTrackerProps {
  ...
  updateMaxGrades: (key: keyof AppSettings['maxGrades'], value: number) => void;
}

export const StudentTracker: React.FC<StudentTrackerProps> = ({ classes, updateStudent, currentWeek, maxGrades, updateMaxGrades }) => {
  ...
  const getStatusColor = (status?: string, currentType?: string) => {
    if (status === currentType) {
        switch(status) {
            case 'present': return 'bg-emerald-600 text-white border-emerald-600 shadow-md scale-105';
            case 'absent': return 'bg-rose-500 text-white border-rose-500 shadow-md scale-105';
            case 'late': return 'bg-amber-500 text-white border-amber-500 shadow-md scale-105';
        }
    }
      ...
      <div className="px-4 lg:px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 border-b border-slate-50 dark:border-slate-800 bg-white/30 dark:bg-slate-900/30 backdrop-blur-sm z-20">
        <div className="flex items-center justify-between w-full md:w-auto gap-3">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="text-sm lg:text-base font-bold text-slate-800 dark:text-white leading-tight">متابعة الطلاب</h2>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-60">
                ...
                <input 
                  ...
                  className="w-full pl-3 pr-9 py-2 rounded-xl text-[12px] font-medium outline-none border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 focus:border-emerald-500 transition-all text-slate-800 dark:text-white shadow-sm" 
                  value={searchQuery} 
            ...
            <button 
              onClick={() => setShowGradeSettings(!showGradeSettings)}
              className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-bold transition-all border ${showGradeSettings ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500'}`}
            >
              <Settings2 className="w-4 h-4" /> إعداد الدرجات
            </button>
        </div>
      </div>
          ...
          {activeClass ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-28">
                {filteredStudents.map((student) => {
                    ...
                    return (
                        <div key={student.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl flex flex-col border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 rounded-xl flex items-center justify-center font-bold text-sm border border-emerald-100 dark:border-emerald-900/20 group-hover:scale-110 transition-transform">
                            ...
                            <div className="flex gap-1.5">
                                {['present', 'absent', 'late'].map((type) => (
                                    <button 
                                        ...
                                        className={`flex-1 py-2 rounded-xl text-[10px] font-bold border transition-all ${getStatusColor(status, type)}`}
                                    >
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
              ...
              <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl relative z-10 overflow-hidden animate-vibrant border border-slate-200 dark:border-slate-800">
                  ...
                  <div className="p-5 bg-slate-50 dark:bg-slate-800/50 flex justify-end">
                      <button onClick={() => setEditingStudent(null)} className="px-8 py-3 bg-emerald-600 text-white rounded-2xl text-[12px] font-bold flex items-center gap-2 shadow-lg shadow-emerald-600/20 active:scale-95 transition-transform">
...
const GradeControl = ({ label, value, icon, onDelta, max }: any) => (
    <div className="space-y-3">
        <div className="flex items-center justify-between">
            ...
            <span className="px-2.5 py-1 bg-emerald-600 text-white rounded-full text-[10px] font-black shadow-sm">{value} / {max}</span>
        </div>
        <div className="flex items-center gap-3">
            <button onClick={() => onDelta(-1)} className="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:border-rose-200 transition-all"><Minus className="w-3.5 h-3.5" /></button>
            <div className="flex-1 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border border-slate-100 dark:border-slate-700">
                <div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${Math.min(100, (value / max) * 100)}%` }}></div>
            </div>
            <button onClick={() => onDelta(1)} className="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 hover:text-emerald-600 hover:border-emerald-200 transition-all"><Plus className="w-3.5 h-3.5" /></button>
        </div>
    </div>
);
